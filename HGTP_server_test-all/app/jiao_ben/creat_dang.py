...
class add_product(unittest.TestCase):
    ...
    def test_creat(self):
        ...
        self.h.find_element_by_partial_link_text('批量新增商品').click()
        ...
        self.s=win_exe()
        time.sleep(1)
        self.s.send(['打开','ComboBoxEx32','ComboBox','Edit','good_data'])
        time.sleep(1)
        self.s.click(['打开','Button'])
        time.sleep(2)
        self.h.find_element_by_xpath('//section[@class="batch-upload-btn"]/button').click()
        while '上传中' in self.h.find_element_by_xpath('//section[@class="batch-upload-btn"]/button').text:
            time.sleep(1)
    @classmethod
