...
class creat(object):
    def __init__(self,s):
        if isinstance(s,webdriver.chrome.webdriver.WebDriver):
            ...
            self.h.find_element_by_partial_link_text('店铺管理').click()
            self.h.find_element_by_partial_link_text('店铺申请').click()
            ...
            self.wait.until(lambda z:z.find_element_by_partial_link_text('审核').is_displayed())
            self.he_zuo_bian_ma=self.h.find_element_by_xpath('//div[@id="div_list"]//tr/td[1]').text
            self.h.find_element_by_partial_link_text('审核').click()
