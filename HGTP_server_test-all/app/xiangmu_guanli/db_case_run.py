...
def add_xiangmu(func):
    def add_xiangmu():
        func()
        xiangmu_name=  request.get_json()['name']
        user_name =g.user_name
        create_time = time.time()
        user = xiangmu_xiangmu_list(name=xiangmu_name, create_time=create_time,
                        create_user=user_name)
        db.session.add(user)
        db.session.flush()
        locust_id = user.id
        db.session.commit()
        return jsonify(status='success',name=xiangmu_name,id=locust_id)
    return add_xiangmu

def get_all_xiangmu(func):
    def get_all_xiangmu():
        func()
        all_data= xiangmu_xiangmu_list.query.filter_by().all()
        return_data=[]
        for i in all_data:
            return_data.append({
                'name':i.name,
        ...
        return jsonify(status='success',return_data = return_data)
    return get_all_xiangmu
