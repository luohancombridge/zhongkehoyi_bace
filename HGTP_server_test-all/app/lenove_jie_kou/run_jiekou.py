             ...
             for i in data:
                 ...
                 for k,z in list(json.loads(i[2]).items()):
                     ...
                     req = json.dumps(json.loads(json.dumps(demjson.decode(json.dumps(req))), parse_int=int), indent=4,
                                    sort_keys=False,
                                    ensure_ascii=False)
                     respons_headers = json.dumps(json.loads(json.dumps(demjson.decode(json.dumps(respons_headers))), parse_int=int), indent=4,
                                    sort_keys=False,
                                    ensure_ascii=False)
                     if z['assert_result']==False:
                         ...
                         fail+=1
                         all_fail+=1
                         detail.append(["failCase",id,comment,case_assert,result,req,req_url,respons_headers])
                     else:
                         all_success+=1
                         succ+=1
                         detail.append(["passCase", id, comment, case_assert,result,req,req_url,respons_headers])
...
def jie_kou_vue(func):
  ...
  def ceshi():
    ...
    if request.method == "GET":
        ...
        #z中元素第一个接口名字，第二个接口的count，第三个用例状态，最后一个列表d第一个为用例状态，第二个用例id，第三个用例comment，第四个用例的接口数据
        db.close()
        return jsonify(z=all,time=tim,success=all_success,fail=all_fail)
  return ceshi
...
def jiekou_result(func):
    def resultee():
        ...
        db.commit()
        for k,i in list(eval(data).items()):
