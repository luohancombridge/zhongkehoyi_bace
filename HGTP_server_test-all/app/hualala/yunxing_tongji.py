...
matplotlib.use('Agg')
from pylab import *
#mpl.rcParams['font.sans-serif'] = ['adadadfadf']
from app import app
    ...
    def yunxing_tongji():
        ...
        if 'ci_seven_data'  in  list(current_app.config.keys()) and request.form['type']=='yewu_seven':
                    if current_app.config['ci_seven_data']['all_num'] == 0:
                        ...
                        tongguolv = int(float(current_app.config['ci_seven_data']['all_pass']) / float(
                            current_app.config['ci_seven_data']['all_num']) * 100)
                    return jsonify(detail=json.dumps(current_app.config['ci_seven_data']['all_git_detail']),
                                   tongguolv=tongguolv)
        elif 'ci_today_data' in list(current_app.config.keys()) and  request.form['type'] == 'yewu_today':
            if  current_app.config['ci_today_data']['all_num']==0:
                tongguolv=0
            else:
                tongguolv=int(float(current_app.config['ci_today_data']['all_pass']) / float(
                               current_app.config['ci_today_data']['all_num']) * 100)
            return jsonify(detail=json.dumps(current_app.config['ci_today_data']['all_git_detail']),
                           tongguolv=tongguolv)
        ...
        for k,i in git_detail:
                    if request.form['type']=='yewu_seven':
                                ...
                                this_git_detail_seven={'name':k,'branch':i,'cishu_num':len(seven_detail),'jiekou_num':0,
                                ...
                                current_app.config['ci_seven_data'] = {'all_git_detail': all_git_detail,
                                                                       'all_fail': all_fail,
                                                                       'all_pass': all_pass, 'all_num': all_num,'time':time.time()}
                    elif request.form['type'] == 'yewu_today':
                        today_detail = cu.execute('select * from ci_tongji where git_url=? and branch=? and time>=?',
                                                  (k, i, today)).fetchall()
                        this_git_detail_today = {'name': k, 'branch': i, 'cishu_num': len(today_detail),
                                                 'jiekou_num': 0,
                                                 "pass_num": 0, "fail_num": 0}
                        max_pass = 0
                        ...
                        all_num = all_fail + all_pass
                        current_app.config['ci_today_data']={'all_git_detail':all_git_detail,'all_fail':all_fail,
                                                             'all_pass':all_pass,'all_num':all_num,'time':time.time()}
        db.close()
        db_jeikou.close()
        if float(all_num) == 0:
        ...
        return jsonify(detail=json.dumps(all_git_detail),
                       tongguolv=tongguolv)
            ...
            if 'today_ci_pic' not in list(current_app.config.keys()) and 'yewu_today' in testid:
                ...
                for k, i in git_detail:
                    ...
                    if len(today_detail)!=0:
                        all_zero = False
                    beizhu = \
                    cu_jiekou.execute('select beizhu from  git_detail  where name=? and branch=?', (k, i)).fetchall()[
                        0][0]
                    for z in today_detail:
                        all_jiekou += int(z[4])
                        all_pass += int(z[6])
                ...
                k = plt.bar(list(range(len(fail_list))), fail_list, label='fail', fc='r')
                d = plt.bar(list(range(len(fail_list))), pass_list, bottom=fail_list, label='pass', tick_label=beizhu_list,
                            fc='y')
                for rect, a, baifenbi in zip(k, d, tongguolv_list):
                    height = rect.get_height() + a.get_height()
                    plt.text(rect.get_x() + rect.get_width() / 2, height, baifenbi, ha='center', va='bottom')
...
def local_tongji_seven(func):
    def local_tongji_seven():
        ...
        seven_day = time.strftime("%Y-%m-%d", time.localtime(seven_day))
        #第二个为多少人用，其余为和
        local_detail = cu.execute(
            'select user_team,count(distinct(name)),sum(pass_jiekou ),sum(fail_jiekou),sum(fail_case_num),sum(pass_case_num) from  local_tongji where time>=? group by user_team',(seven_day,)).fetchall()
...
def local_seven_pic(func):
    def local_seven_pic(testid):
        ...
        if    'local_seven_pic'  not in list(current_app.config.keys()):
            ...
            # 第二个为多少人用，其余为和
            local_detail = cu.execute(
                'select user_team,count(name),sum(num ),sum(fail_jiekou),sum(fail_case_num),sum(pass_case_num) from  local_tongji where time>=? group by user_team',
            ...
            #plt.close()
        elif 'local_seven_pic' in list(current_app.config.keys()) and 'local_seven_pic' in testid:
            pass
...
def local_tongji_today(func):
    def local_tongji_today():
        ...
        #第二个为多少人用，其余为和
        local_detail = cu.execute(
            'select user_team,count(distinct(name)),sum(pass_jiekou ),sum(fail_jiekou),sum(fail_case_num),sum(pass_case_num) from  local_tongji where time>=? group by user_team',(today,)).fetchall()
        return jsonify(local_detail=json.dumps(local_detail))
...
def local_today_pic(func):
    def local_today_pic(testid):
        ...
        if 'local_today_pic' not in list(current_app.config.keys()):
            ...
            # 第二个为多少人用，其余为和
            local_detail = cu.execute(
                'select user_team,count(name),sum(pass_jiekou ),sum(fail_jiekou),sum(fail_case_num),sum(pass_case_num) from  local_tongji where time>=? group by user_team',
                (today_day,)).fetchall()
        ...
        elif 'local_today_pic' in list(current_app.config.keys()) and 'local_today_pic' in testid:
...
def pic_yewu_email(testid):
            ...
            d = plt.bar(list(range(len(fail_list))), pass_list, bottom=fail_list, label='pass', tick_label=git_detail,
                        fc='#66CC66',width=0.2)
            for rect, a, baifenbi in zip(k, d, tongguolv_list):
                height = rect.get_height() + a.get_height()
                plt.text(rect.get_x() + rect.get_width() / 2, height,baifenbi, ha='center', va='bottom')
        ...
        def pic_yewu_today_email():
                ...
                for k, i in git_detail:
                    ...
                    if len(today_detail) != 0:
                        all_zero = False
                    beizhu = \
                        cu_jiekou.execute('select beizhu from  git_detail  where name=? and branch=?',
                                          (k, i)).fetchall()[
                            0][0]
                    for z in today_detail:
                        all_jiekou += int(z[4])
